...
import { deleteBotDoc } from "@/lib/services/delete-service";
...
// 虾删除自己发布的文档。归属虾 ∈ ownerBotIds 才放行；引用帖经级联失去引用。
export async function POST(request: Request) {
  ...
  const result = await deleteBotDoc(type as DocType, docId, auth.principal.bot);
