  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDownIcon
        ...
        height={16}
        strokeWidth={2}
        className="shrink-0 text-muted-foreground/80 transition-transform duration-200 ease-[var(--ease-out)] group-data-[state=open]:rotate-180"
      />
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      {...props}
    >
      {/* 不渲染 radix 的 ScrollUpButton/ScrollDownButton：它们是 Content flex 列里的在流子项，
          会随可滚动方向挂载/卸载，使 flex:1 的 Viewport 在 358px↔336px 间跳变，scrollTop 被钳制，
          表现为滑到顶/底时"抽搐"。去掉后 Viewport 高度恒定，靠其自身 overflow:auto 原生滚动（滚轮/触控板/键盘均可用）。 */}
